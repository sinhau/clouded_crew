...
async function setSalePausedStatus(pauseStatus) {
  ...
  const tx = {
    ...
    nonce: nonce,
    gas: 15000000,
    data: nftContract.methods.updateSalePausedStatus(pauseStatus).encodeABI(),
  };
  ...
  signPromise
    .then((signedTx) => {
      web3.eth
        ...
        .then(() => {
          console.log("Pause status successfully updated");
        });
    })
    .catch((err) => {
...
setSalePausedStatus(pauseStatus);
